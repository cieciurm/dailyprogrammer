...
namespace Mateusz.DailyProgrammer._257
{
    public class Reader
    {
        public IEnumerable<President> GetPresidents()
        {
            ...
            using (var reader = new StringReader(Input))
            {
                ...
                while ((line = reader.ReadLine()) != null)
                {
                    ...
                    var name = trimmedItems[0];
                    var dateOfBirth = DateTime.Parse(trimmedItems[1]);

                    DateTime? dateOfDeath;
                    if (trimmedItems[3] == string.Empty)
                    {
                        dateOfDeath = null;
                    }
                    else
                    {
                        dateOfDeath = DateTime.Parse(trimmedItems[3]);
                    }

                    list.Add(new President(name, dateOfBirth, dateOfDeath));
        ...
        private const string Input = @"PRESIDENT,  BIRTH DATE, BIRTH PLACE,    DEATH DATE, LOCATION OF DEATH
